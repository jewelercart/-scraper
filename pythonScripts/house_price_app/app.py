...
app = Flask(__name__)
app.secret_key = "jkhioujoi2je9u2eiwue092ue92"

DB_CONFIG = {
    "host": "localhost",
    "user": "root",
    "password": "NewPass!123",
    "database": "housecashprohome_db"
}
...
try:
    ...
    print(f"[WARNING] DB init failed (check firewall/connection): {e}")
...
def signup():
    if request.method == "POST":
        ...
        beds = int(float(request.form["beds"])) if request.form["beds"].strip() else None
        baths = int(float(request.form["baths"])) if request.form["baths"].strip() else None
        sqft = int(float(request.form["sqft"])) if request.form["sqft"].strip() else None
        lot_size = int(float(request.form["lot_size"])) if request.form["lot_size"].strip() else None
        year_built = int(float(request.form["year_built"])) if request.form["year_built"].strip() else None
        ...
        # Predict house value
        try:
            features = [[beds or 0, baths or 0, sqft or 0, lot_size or 0, year_built or 0]]
